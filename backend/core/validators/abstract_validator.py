...
_KEYWORDS_ANYWHERE_NEAR_START = re.compile(
...
class AbstractValidator:
    ...
    def _check_not_keywords(text: str, weight: float) -> RuleResult:
        """Reject a keyword block leaked into the beginning of an abstract."""
        ...
        # A broken reading order can leave the tail of an abstract before its
        # keyword line and then continue with another section/language.  The
        # label is not at character zero in that case, but it is still evidence
        # of a contaminated extraction rather than a self-contained abstract.
        leaked = _KEYWORDS_ANYWHERE_NEAR_START.search(text[:350])
        if leaked:
            return RuleResult(
                rule_name="not_keywords",
                passed=False,
                score=0.0,
                weight=weight,
                message="abstract contains a Keywords/Index Terms block near its start",
            )
        return RuleResult(
