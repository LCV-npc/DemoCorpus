...
_FOOTNOTE_PATTERN = re.compile(
    # PDF fonts sometimes encode an asterisk or superscript in Unicode's
    # private-use range (for example U+F02A). Such glyphs are not names.
    r"[¹²³⁴⁵⁶⁷⁸⁹⁰ᵃᵇᶜᵈᵉ*†‡§¶‖#\uE000-\uF8FF]+",
)
...
# The extractor can join two author spans with only an affiliation marker
# between them: "Nguyễn Văn A1 Trần Thị B".
...
_SEPARATOR_PATTERN = re.compile(
    ...
    |\s*\bv(?:\u00e0|a)\b\s*  # Vietnamese "và" / unaccented "va"
    |\s*&\s*             # ampersand
...
class AuthorCleaner:
    ...
    def split_and_clean(self, raw: str) -> list[str]:
        ...
        text = _INLINE_AFFILIATION_BOUNDARY.sub(", ", text)
    ...
    def clean_name(self, name: str) -> str:
        ...
        # A detached affiliation marker may remain after a column split.
    ...
    def _has_valid_structure(name: str) -> bool:
        ...
        # ≥ 2 tokens.  A real author line can contain particles ("de", "van")
        # but a prose sentence or dosage must not be accepted as a person name.
        tokens = name.split()
        ...
        # Footnote digits have already been removed above.  Remaining digits
        # are strong evidence of body text (e.g. "nhiều nhất là 400ml"), not a
        # personal name.
        if any(c.isdigit() for c in name):
            return False

        # Vietnamese and Western researcher names normally retain uppercase
        # initials.  Require two capitalized tokens, while allowing particles
        # such as "de" or "van" between them.
        capitalized = sum(token[0].isupper() for token in tokens if token)
        if capitalized < min(2, len(tokens)):
            return False
